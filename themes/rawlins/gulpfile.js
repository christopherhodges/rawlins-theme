...
gulp.task('scss-admin', function () {
	return sass(themePath + 'css/admin/style-admin.scss', { sourcemap: false })
		.on('error', sass.logError)
		.pipe(autoprefixer('last 4 version'))
		.pipe(minifyCSS({keepBreaks:false}))
		.pipe(rename({suffix: '.min' }))
		.pipe(gulp.dest( themePath + 'css/admin/' ))
		.pipe(notify({ message: 'Scss-admin task complete' }));
});

gulp.task('scss-login', function () {
	return sass(themePath + 'css/admin/style-login.scss', { sourcemap: false })
		.on('error', sass.logError)
		.pipe(autoprefixer('last 4 version'))
		.pipe(minifyCSS({keepBreaks:false}))
		.pipe(rename({suffix: '.min' }))
		.pipe(gulp.dest( themePath + 'css/admin/' ))
		.pipe(notify({ message: 'Scss-login task complete' }));
});

gulp.task('scss-tinymce', function () {
	return sass(themePath + 'css/admin/style-tinymce.scss', { sourcemap: false })
		.on('error', sass.logError)
		.pipe(autoprefixer('last 4 version'))
		.pipe(minifyCSS({keepBreaks:false}))
		.pipe(rename({suffix: '.min' }))
		.pipe(gulp.dest( themePath + 'css/admin/' ))
		.pipe(notify({ message: 'Scss-tinymce task complete' }));
});
...
gulp.task( 'watch', function() {
	...
	gulp.watch( themePath + '**/**/*.scss', [ 'scss', 'scss-admin', 'scss-login', 'scss-tinymce' ] );
